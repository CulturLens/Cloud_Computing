...
router.post(
  ...
  async (req, res) => {
    ...
    db.query(
      query,
      [name, email, hashedPassword, username, profilePhoto],
      (err, result) => {
        if (err)
          return res
            .status(500)
...
router.put(
  ...
  async (req, res) => {
    ...
    db.query(
      "SELECT username, email, password, phone, profilePhoto FROM users WHERE id = ?",
      [id],
      (err, result) => {
        if (err) {
          return res
        ...
        if (req.body.username && req.body.username !== currentUser.username) {
          return res
            .status(400)
            .json({ message: "Username cannot be changed" });
        }
...
router.delete("/user/:id", (req, res) => {
  ...
  db.query(deletePostsQuery, [id], (err) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error deleting user posts", error: err });
    }
    ...
    db.query(deleteCommentsQuery, [id], (err) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error deleting user comments", error: err });
      }
  ...
  try {
    ...
    db.query(getUserIdQuery, [username], (err, userResult) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error fetching user_id", error: err });
      }
      ...
      db.query(
        insertForumQuery,
        [title, description, username, image, user_id, currentTime],
        (err, forumResult) => {
          if (err) {
            return res
              .status(500)
              .json({ message: "Error creating forum post", error: err });
          }
...
router.post("/comment", (req, res) => {
  ...
  db.query(getForumOwnerQuery, [postId], (err, forumResult) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error fetching forum", error: err });
    }
    ...
    if (!recipientId) {
      return res
        .status(404)
        .json({ message: "Recipient (forum owner) not found" });
    }
    ...
    db.query(getRecipientQuery, [recipientId], (err, recipientResult) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error fetching recipient data", error: err });
      }
      ...
      `;
        ...
        db.query(
          insertCommentQuery,
          [comment, postId, userId, currentTime],
          (err, insertResult) => {
            if (err) {
              console.error("Error inserting comment:", err);
              return res
                .status(500)
                .json({ message: "Error inserting comment", error: err });
            }
        ...
        db.query(
          ...
          (err, insertNotificationResult) => {
            if (err) {
              ...
              return res
                .status(500)
                .json({ message: "Error inserting notification", error: err });
            }
...
// Rute GET /comment untuk mendapatkan semua komentar atau komentar berdasarkan postId
router.get('/comment', (req, res) => {
  const { postId } = req.query; // Mendapatkan postId dari query parameter

  let getCommentsQuery = `
    SELECT comments.id, comments.comment, comments.created_at, 
           users.name AS commenterName, users.profilePhoto AS commenterPhoto
    FROM comments
    JOIN users ON comments.user_id = users.id
  `;

  if (postId) {
    getCommentsQuery += ` WHERE comments.post_id = ?`;
  }

  getCommentsQuery += ` ORDER BY comments.created_at DESC`;

  db.query(getCommentsQuery, postId ? [postId] : [], (err, comments) => {
    if (err) {
...
// Rute GET /comment/:commentId untuk mendapatkan komentar berdasarkan commentId
router.get('/comment/:commentId', (req, res) => {
  const { commentId } = req.params; // Mendapatkan commentId dari parameter URL
  ...
  db.query(getCommentQuery, [commentId], (err, comment) => {
    if (err) {
      console.error("Error fetching comment:", err);
      return res.status(500).json({ message: "Error fetching comment", error: err });
    }
...
// Rute DELETE /comment untuk menghapus komentar berdasarkan commentId
...
// Rute POST /comment untuk menambah komentar dan membuat notifikasi
router.post('/comment', (req, res) => {
  const { comment, postId, userId } = req.body; // Mendapatkan data dari body
  const currentTime = new Date().toISOString().slice(0, 19).replace('T', ' ');

  if (!comment || !postId || !userId) {
    return res.status(400).json({ message: "comment, postId, and userId are required" });
  }

  // Dapatkan pemilik forum berdasarkan postId
  const getForumOwnerQuery = `SELECT user_id FROM forums WHERE id = ?`;
  db.query(getForumOwnerQuery, [postId], (err, forumResult) => {
    if (err) {
      console.error("Error fetching forum:", err);
    ...
    const recipientId = forumResult[0].user_id;
    ...
    db.query(insertCommentQuery, [comment, postId, userId, currentTime], (err, insertResult) => {
      ...
      // Buat notifikasi komentar
      const notificationQuery = `
        INSERT INTO notifications 
        (title, message, user_id, action_type, created_at, recipient_id) 
        VALUES (?, ?, ?, 'comment', ?, ?)
      `;

      const title = `New comment on your post`;
      const message = `User ${userId} commented: "${comment}"`;
      ...
      db.query(getUserQuery, [userId], (err, userResult) => {
        ...
        db.query(
          ...
          (err, insertResult) => {
            ...
            res.status(200).json({
              message: "Like added and notification created successfully",
            });
          }
        );
      });
...
router.get("/forums/images", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error fetching forum images", error: err });
    }
  ...
  db.query(query, (err, results) => {
    ...
    const forums = results.map((forum) => ({
      ...
      imageUrl: forum.image
        ? `${baseUrl}/${forum.image.replace(/\s+/g, "%20")}` // Ganti spasi di respons
        : null,
    }));
...
router.get("/forum/:id", (req, res) => {
  ...
  const query = `SELECT id, title, description, username, image FROM forums WHERE id = ?`;
  db.query(query, [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ message: "Error fetching forum data", error: err });
    ...
    const forum = result[0]; // Mengambil forum pertama

    // Link gambar dengan backslash tetap ada (menggunakan path yang sudah ada)
    const imageUrl = forum.image
      ? `${baseUrl}/${forum.image}` // Menjaga backslash dalam URL
      : null; // Fallback jika image null

    // Menyusun respons yang diinginkan
    res.status(200).json({
      ...
      forum: [
        {
          ...
          description: forum.description,
          username: forum.username,
          imageUrl: imageUrl, // Link gambar dengan backslash
        }
      ]
    });
  });
  ...
  db.query(deleteCommentsQuery, [id], (err) => {
    ...
    db.query(deleteForumQuery, [id], (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error deleting forum post", error: err });
      }
      if (result.affectedRows === 0) {
...
router.post("/notification", authenticateToken, (req, res) => {
  ...
  db.query(
    query,
    [title, message, user_id, activity, profile_photo],
    (err, result) => {
      if (err)
        return res
          .status(500)
...
router.get("/likes/:post_id", (req, res) => {
  ...
  db.query(query, [post_id], (err, results) => {
    if (err) {
      console.error("Error fetching likes for post:", err.message); // Log lebih detail
      return res.status(500).json({ message: "Error fetching likes", error: err.message });
    }

    if (results.length === 0) {
      return res.status(404).json({ message: `No likes found for post with id ${post_id}` });
    }
...
router.get("/likes/user/:user_id", (req, res) => {
  ...
  db.query(query, [user_id], (err, results) => {
    if (err) {
      console.error("Error fetching likes for user:", err.message); // Log lebih detail
      return res.status(500).json({ message: "Error fetching user likes", error: err.message });
    }

    if (results.length === 0) {
      return res.status(404).json({ message: `No likes found for user with id ${user_id}` });
    }
...
router.delete("/like/:id", (req, res) => {
  ...
  db.query(query, [id], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error deleting like", error: err });
    }
    if (result.affectedRows === 0) {
